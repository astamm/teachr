...
```{r clean-www, include=FALSE}
if (file.exists("www/df.csv"))
  file.remove("www/df.csv")
```
...
list.files("www/")
readr::write_csv(df, path = "www/df.csv")
list.files("www/")
writeLines(system("cat www/df.csv", intern = TRUE))
```

### Exercise

- Import the `nobel.csv` data from the URL (https://github.com/astamm/teachr/tree/master/inst/tutorials/03_DataImport/www/nobel.csv);
...
```{r variable-names}
edi_airbnb <- readr::read_csv("www/edi-airbnb.csv")
names(edi_airbnb)
```
...
```{r names-o2}
edi_airbnb_col_names <- readr::read_csv("www/edi-airbnb.csv",
