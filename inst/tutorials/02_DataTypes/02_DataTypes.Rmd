...
3.  If you did not expect this result, what piece of `R`{=html} code would compute what you intended?
...
Atomic vectors can only store booleans, numeric entries or strings and are required to contain only one type of these. In fact, `R`{=html} is very permissive and allows you to mix all types into a vector but results might be unexpected:
...
2.  Creating vectors with homogeneous entries that have more complex data types, such as matrices, data sets, results as output by `R`{=html} modeling functions or any other `R`{=html} object.
...
-   `[` always return a list even when a single element of the original list is requested while `[[` converts the result into the appropriate simpler `R`{=html} object;
