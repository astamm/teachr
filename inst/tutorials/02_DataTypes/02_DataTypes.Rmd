...
To create a matrix from a data set made of numerical variables, use `as.matrix()`. For example, the first $4$ variables in the `iris` data set are numeric. We can therefore coerce them into a matrix as follows:
```{r as-matrix}
...
```{r dim, exercise=TRUE, exercise.setup="def-X"}

```

To transpose a matrix, use `t()`:
```{r transpose, exercise=TRUE, exercise.setup="def-X"}

```

Universal mathematical functions and operators `+`, `-`, `*` and `/` can be used without any problem. They just apply the function or the operator element-wise. For operations, matrix dimensions should match. An error will be thrown if it is not the case. The element-wise multiplication is not the usual matrix multiplication. The latter can be performed using the operator `%*%`:
```{r matrix-product, exercise=TRUE, exercise.setup="def-X"}
# t(X) * X
A <- t(X) %*% X
A
```
...
```{r solve, exercise=TRUE, exercise.setup="def-A", exercise.lines=5}
# Solve Ax = b
solve(A, rep(1, 4))
# Find inverse of A
solve(A)
...
```{r mat-stat, exercise=TRUE, exercise.setup="def-A"}
colMeans(A)
cov(A)
