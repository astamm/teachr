...
- **r.squared** and **adj.r.squared** (TO DO: see `qnscombe` data set for caveat of using it alone). The coefficient of determination $R^2$ and its adjusted version $R_\mathrm{adj}^2$ aim at estimating the proportion of variance of the response variable $\mathbf{Y}$ explained by the fitted model $\widehat{\mathbf{Y}}$. Mathematically, they aim at estimating 
