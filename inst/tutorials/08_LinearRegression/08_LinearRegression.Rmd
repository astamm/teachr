...
- **r.squared** and **adj.r.squared** (TO DO: see `anscombe` data set for caveat of using it alone). The coefficient of determination $R^2$ and its adjusted version $R_\mathrm{adj}^2$ aim at estimating the proportion of variance of the response variable $\mathbf{Y}$ explained by the fitted model $\widehat{\mathbf{Y}}$. Mathematically, they aim at estimating 
...
$$ \mathrm{Var}[\widehat{\beta}_j] = \frac{\sigma^2}{\sum_{i=1}^n x_{ij}^2}. $$

The `std.error` is the square root of an estimate of that variance produced by the following estimator:

$$ \widehat{\mathrm{Var}[\widehat{\beta}_j]} = \frac{\widehat{\sigma^2}}{\sum_{i=1}^n x_{ij}^2} = \frac{1}{n - k - 1} \frac{\widehat{\boldsymbol{\varepsilon}}^\top \widehat{\boldsymbol{\varepsilon}}}{\mathbf{x}_j^\top \mathbf{x}_j}. $$
...
There is no built-in `geom_()` function in [**ggplot2**](https://ggplot2.tidyverse.org) for visualizing interactions.

TO DO: introduce the modelr package somewhere
...
TO DO: mention the [**oslrr**](https://www.r-bloggers.com/introducing-olsrr/) package somewhere.
...
TO DO: add exercises + anscombe ? + VIF ?
