...
df <- tibble::tibble(x, first_data, second_data) %>%
...
```{r df-non-linear-viz, fig.align='center'}
df %>%
...
```{r diagnose-mod1, fig.align='center', exercise=TRUE, exercise.setup = "df-non-linear-lm"}

```

Let us now diagnose the second model:

```{r diagnose-mod2, fig.align='center', exercise=TRUE, exercise.setup = "df-non-linear-lm"}
